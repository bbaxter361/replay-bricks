...
test('portal embeds the current Compass v2 frontend', async () => {
  ...
  assert.match(source, /https:\/\/compass-replaybricks-v2-550\.netlify\.app\?auto=true/);
  assert.doesNotMatch(source, /https:\/\/compass-replaybricks\.netlify\.app\?auto=true/);
});
