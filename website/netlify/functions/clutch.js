...
// Strip /hold prefix when behind Netlify redirect (/api/clutch/* → function)
// Express routes are mounted at /api/* so we need to rewrite the URL
// MUST run BEFORE auth middleware so req.path is clean
//
// Netlify rewrites /api/clutch/* → /.netlify/functions/clutch
// The function receives event.path = "/.netlify/functions/clutch" (the rewritten path).
// serverless-http sets req.url = event.path, so we see /api/clutch/* ONLY when
// accessed via a different routing layer. In production Netlify, req.url is the
// rewritten path — strip that prefix too.
app.use((req, res, next) => {
  if (req.url.startsWith('/.netlify/functions/clutch/')) {
    req.url = req.url.replace('/.netlify/functions/clutch/', '/api/');
  } else if (req.url.startsWith('/.netlify/functions/clutch')) {
    req.url = req.url.replace('/.netlify/functions/clutch', '/api');
...
const API_TOKEN = (() => {
  ...
  return crypto.createHash('sha256').update('clutch-replay-bricks-' + (process.env.NETLIFY_SITE_ID || 'local')).digest('hex').slice(0, 48);
})();
...
async function getBlobStore() {
  ...
  try {
    const store = getStore('clutch-data');
...
// Migration map: old hold_* keys → new clutch_* keys
const MIGRATION_MAP = {
  clutch_inventory: 'hold_inventory',
  clutch_orders: 'hold_orders', 
  clutch_order_items: 'hold_order_items',
  clutch_bl_colors: 'hold_bl_colors',
  clutch_api_credentials: 'hold_api_credentials',
  clutch_sync_log: 'hold_sync_log',
  clutch_marketplace_lots: 'hold_marketplace_lots',
  clutch_pricing_rules: 'hold_pricing_rules',
  clutch_price_cache: 'hold_price_cache',
  clutch_pending_items: 'hold_pending_items',
  clutch_settings: 'hold_settings',
  clutch_push_log: 'hold_push_log',
};

async function blobRawGet(key) {
  const store = await getBlobStore();

  // Try new clutch_* key first
  let data;
  if (store instanceof Map) {
    const raw = store.get(key);
    if (raw) data = JSON.parse(raw);
  } else {
    const fetched = await store.get(key, { type: 'json' });
    if (fetched) data = fetched;
  }

  // If empty, try migrating from old hold_* key
  if ((!data || (Array.isArray(data) && data.length === 0)) && MIGRATION_MAP[key]) {
    const oldKey = MIGRATION_MAP[key];
    let oldData;
    if (store instanceof Map) {
      const raw = store.get(oldKey);
      if (raw) oldData = JSON.parse(raw);
    } else {
      const fetched = await store.get(oldKey, { type: 'json' });
      if (fetched) oldData = fetched;
    }
    
    if (oldData && (!Array.isArray(oldData) || oldData.length > 0)) {
      // Migrate it to new key
      if (store instanceof Map) {
        store.set(key, JSON.stringify(oldData));
      } else {
        await store.setJSON(key, oldData);
      }
      console.log(`Migrated ${oldKey} → ${key} (${Array.isArray(oldData) ? oldData.length : 1} records)`);
...
/**
 * Batch-update multiple items in a single read-modify-write cycle.
 * updates: [{ id, data: { field: value, ... } }]
 * Returns array of updated ids.
 * This is the key performance fix — avoids N individual reads/writes.
 */
async function blobBatchUpdate(key, updates) {
 ...
 * Rejects with a descriptive error if `ms` milliseconds pass before `promise` settles.
 * Prevents Netlify function from exceeding the 90-second hard limit (80s used here).
 */
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`Operation timed out after ${ms}ms${label ? ': ' + label : ''}`)), ms)
    ),
  ]);
}
...
function computeImageUrls(partNo, colorId, hasPartName) {
  // Use SN (Set Image) for named sets, PN (Part Image) for unnamed parts
  const isSet = hasPartName || /-\d+$/.test(partNo);
...
async function loadBLCredentials() {
  ...
  const creds = await blobRawGet('clutch_api_credentials');
...
async function syncLogStart(marketplace, syncType) {
  ...
  return await blobInsert('clutch_sync_log', entry);
}

async function syncLogComplete(logId, status, count, errors) {
  await blobUpdate('clutch_sync_log', logId, {
    status,
    ...
    while (hasMore) {
      ...
      for (const item of items) {
        try {
          ...
          const inventory = await blobRawGet('clutch_inventory');
          let existing;
          if (colorId !== null) {
          ...
          let invId;
          if (existing) {
            await blobUpdate('clutch_inventory', existing.id, {
              quantity: qty,
            ...
            const created = await blobInsert('clutch_inventory', {
              part_no: partNo,
              color_id: colorId,
              part_name: partName,
              quantity: qty,
          ...
          // Fetch catalog info for year/description (best-effort, only if missing)
          if (partName && !existing?.year) {
            try {
              const isSet = /-\d+$/.test(partNo);
              if (isSet) {
                const barePart = partNo.replace(/-\d+$/, '');
                const catalog = await blApiGet(`/items/SET/${barePart}`);
                if (catalog) {
                  const catYear = catalog.year_released ? parseInt(catalog.year_released) : null;
                  const catDesc = catalog.description || null;
                  const generatedDesc = catDesc || `LEGO set: ${partName}`;
                  // Update year and description on the inventory item
                  const inv = await blobRawGet('clutch_inventory');
                  const idx = inv.findIndex(i => i.id === invId);
                  if (idx !== -1) {
                    inv[idx].year = catYear;
                    inv[idx].description = generatedDesc;
                    await blobRawSet('clutch_inventory', inv);
                  }
                }
              }
            } catch (e) {
              // Best-effort — catalog fetch failure shouldn't break sync
            }
          }
          
          // Upsert marketplace lot
          const lots = await blobRawGet('clutch_marketplace_lots');
          ...
          if (existingLot) {
            await blobUpdate('clutch_marketplace_lots', existingLot.id, lotData);
          } else {
            await blobInsert('clutch_marketplace_lots', lotData);
          }
    ...
    while (hasMore) {
      ...
      for (const order of orders) {
        try {
          ...
          const existingOrders = await blobRawGet('clutch_orders');
          const existing = existingOrders.find(o => o.marketplace === 'bricklink' && o.order_id === orderId);
          
          if (existing) {
            await blobUpdate('clutch_orders', existing.id, orderData);
          } else {
            await blobInsert('clutch_orders', orderData);
          }
          ...
          if (orderId_db) {
            try {
              ...
              for (const item of items) {
                ...
                await blobInsert('clutch_order_items', {
                  order_id: orderId_db,
                  marketplace: 'bricklink',
          ...
          // Fetch order detail for shipping address (BrickLink list endpoint doesn't include full address)
          if (!orderData.shipping_address || orderData.shipping_address === '' || orderData.shipping_address === '{}') {
            try {
              const detail = await blApiGet(`/orders/${orderId}`);
              // BrickLink order detail: shipping.address.{name.full, address, address2, city, state, postal_code, country_code}
              const shipAddr = detail?.shipping?.address || detail?.shipping_address;
              if (shipAddr) {
                const addr = typeof shipAddr === 'string' 
                  ? shipAddr 
                  : JSON.stringify(shipAddr);
                const currentOrders = await blobRawGet('clutch_orders');
                const ord = currentOrders.find(o => o.marketplace === 'bricklink' && o.order_id === orderId);
                if (ord) {
                  await blobUpdate('clutch_orders', ord.id, { shipping_address: addr });
                }
              }
            } catch (addrErr) {
              // Non-critical — order sync continues without address
              console.error(`Address fetch failed for BL ${orderId}:`, addrErr.message);
            }
          }
        } catch (err) {
  ...
  try {
    ...
    for (const order of orders) {
      try {
        ...
        // Fetch order detail for shipping address (BrickOwl list endpoint may not include full address)
        // Only fetch if blob still has no address (avoid redundant calls)
        try {
          const currentOrders = await blobRawGet('clutch_orders');
          const ord = currentOrders.find(o => o.marketplace === 'brickowl' && o.order_id === orderId);
          if (ord && (!ord.shipping_address || ord.shipping_address === '')) {
            const detail = await boApiGet('/order/view', { order_id: orderId });
            if (detail) {
              const shipTo = detail.ship_to || detail.shipping_address || detail.address || '';
              if (shipTo && shipTo.length > 5) {
                const addr = typeof shipTo === 'string' ? shipTo : JSON.stringify(shipTo);
                await blobUpdate('clutch_orders', ord.id, { shipping_address: addr });
              } else {
                addrErrors.push(`BO ${orderId}: no address in detail response (keys: ${Object.keys(detail).slice(0,8).join(',')})`);
              }
            }
          }
        } catch (addrErr) {
          addrErrors.push(`BO ${orderId}: ${addrErr.message}`);
        }
    ...
    await syncLogComplete(logId, 'success', count, errors);
    return { synced: count, errors: errors.length > 0 ? errors : undefined, addrErrors: addrErrors.length > 0 ? addrErrors : undefined };
  } catch (err) {
    await syncLogComplete(logId, 'failed', count, [err.message]);
    return { synced: count, errors: [err.message], addrErrors: addrErrors.length > 0 ? addrErrors : undefined };
  }
}
...
app.post('/api/credentials', async (req, res) => {
  try {
    ...
    const existing = await blobFind('clutch_api_credentials', c => c.marketplace === marketplace);
    if (existing) {
      await blobUpdate('clutch_api_credentials', existing.id, {
        credentials: JSON.stringify(credentials),
        updated_at: new Date().toISOString()
      });
    } else {
      await blobInsert('clutch_api_credentials', {
        marketplace,
        credentials: JSON.stringify(credentials),
        updated_at: new Date().toISOString()
      });
...
app.delete('/api/credentials/:marketplace', async (req, res) => {
  try {
    const cred = await blobFind('clutch_api_credentials', c => c.marketplace === req.params.marketplace);
    if (cred) {
      await blobDelete('clutch_api_credentials', cred.id);
    }
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/sync/logs', async (req, res) => {
  try {
    ...
    const logs = await blobRawGet('clutch_sync_log');
...
app.get('/api/inventory', async (req, res) => {
  try {
    ...
    const result = paged.map(item => {
      ...
      const { image_url, catalog_url } = computeImageUrls(item.part_no, item.color_id, !!item.part_name);
      ...
      return {
        ...
        marketplace_count,
        image_url: item.custom_image_url || image_url,
        catalog_url,
        description: item.description || (item.part_name ? `LEGO set: ${item.part_name}` : null),
      };
    });
...
app.get('/api/inventory/export/csv', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="clutch-inventory.csv"');
    res.send(csv);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put('/api/inventory/:id', async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { quantity, unit_price_cents, location, notes, condition, custom_image_url, year, description } = req.body;
    ...
    const success = await blobUpdate('clutch_inventory', id, updates);
...
app.post('/api/inventory', async (req, res) => {
  try {
    ...
    const item = await blobInsert('clutch_inventory', {
      part_no,
...
app.delete('/api/inventory/:id', async (req, res) => {
  try {
    ...
    if (lotsChanged) await blobRawSet('clutch_marketplace_lots', lots);

    await blobDelete('clutch_inventory', id);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/inventory/:id/prices', async (req, res) => {
  try {
    ...
    // Check cache
    const cache = await blobRawGet('clutch_price_cache');
    ...
    if (blCreds) {
      try {
        ...
        if (response.ok) {
          ...
          cache.push(entry);
          await blobRawSet('clutch_price_cache', cache);
...
// POST /api/orders — Create a new storefront order (marketplace: replaybrick)
app.post('/api/orders', async (req, res) => {
  try {
    const { customer_name, customer_email, customer_phone, shipping_address, items, marketplace = 'replaybrick', status = 'paid', notes } = req.body || {};

    if (!customer_name || !customer_email || !shipping_address) {
      return res.status(400).json({ error: 'customer_name, customer_email, and shipping_address are required' });
    }
    if (!Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ error: 'items array is required' });
    }

    // Validate each item has inventory_id and quantity
    for (const item of items) {
      if (!item.inventory_id || !item.quantity || item.quantity < 1) {
        return res.status(400).json({ error: 'Each item must have inventory_id and quantity > 0' });
      }
    }
    ...
    for (const item of items) {
      const invItem = invById.get(item.inventory_id);
      if (!invItem) {
        return res.status(400).json({ error: `Inventory item ${item.inventory_id} not found` });
      }
      if ((invItem.quantity || 0) < item.quantity) {
        return res.status(409).json({
          error: `Insufficient stock for ${invItem.part_no || 'item'}: need ${item.quantity}, have ${invItem.quantity}`,
          shortages: [{ inventory_id: item.inventory_id, part_no: invItem.part_no, available: invItem.quantity, requested: item.quantity }],
        });
      }

      const lineTotal = (invItem.unit_price_cents || 0) * item.quantity;
      ...
      orderItems.push({
        ...
        color_id: invItem.color_id,
        color_name: item.color_name || invItem.color_name || null,
        condition: invItem.condition || 'USED',
        quantity: item.quantity,
        unit_price_cents: invItem.unit_price_cents || 0,
        line_total_cents: lineTotal,
      });
    }

    // Deduct inventory (batch update)
    const inventoryUpdates = items.map(item => ({
      id: item.inventory_id,
      data: {
        quantity: (invById.get(item.inventory_id).quantity || 0) - item.quantity,
        last_synced_at: new Date().toISOString(),
      },
    }));
    await blobBatchUpdate('clutch_inventory', inventoryUpdates);

    // Create order record
    const order = await blobInsert('clutch_orders', {
      marketplace,
      order_id: null, // internal order, no external order_id
      buyer_name: customer_name,
      buyer_email: customer_email,
      buyer_phone: customer_phone || null,
      buyer_notes: notes || null,
      status,
      total_items: items.reduce((sum, i) => sum + (i.quantity || 0), 0),
      total_price_cents: totalPriceCents,
      shipping_cents: 0,
      currency: 'USD',
      shipping_address,
    ...
    try {
      const emailHtml = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #1a1a1a; color: #fff; padding: 30px;">
          <div style="text-align: center; margin-bottom: 20px;">
            <h1 style="color: #E3000B; font-size: 24px; margin: 0;">Replay Bricks</h1>
            <p style="color: #999; font-size: 14px;">Order Confirmation</p>
          </div>
          <p style="font-size: 16px;">Hi ${customer_name.split(' ')[0]},</p>
          <p style="color: #ccc; font-size: 14px;">Thank you for your order! We've received it and will begin processing shortly.</p>
          <div style="background: #242424; border: 1px solid #333; border-radius: 8px; padding: 20px; margin: 20px 0;">
            <p style="color: #999; font-size: 12px; margin: 0 0 10px;">Order #${order.id}</p>
            ${orderItems.map(oi => `
              <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #333; font-size: 13px;">
                <span style="color: #ccc;">${oi.quantity}x ${oi.part_no} — ${oi.part_name || ''}</span>
                <span style="color: #fff;">$${(oi.line_total_cents / 100).toFixed(2)}</span>
              </div>
            `).join('')}
            <div style="display: flex; justify-content: space-between; padding: 12px 0 0; font-size: 16px; font-weight: bold;">
              <span style="color: #fff;">Total</span>
              <span style="color: #E3000B;">$${(totalPriceCents / 100).toFixed(2)}</span>
            </div>
          </div>
          <div style="color: #999; font-size: 12px; margin-top: 20px;">
            <p style="margin: 0 0 5px; color: #ccc;">Shipping to:</p>
          ...
          <p style="color: #666; font-size: 11px; margin-top: 30px; text-align: center;">
            Replay Bricks — Portland, OR<br>
            Questions? Contact us at brian@replaybrick.com
          </p>
        </div>
      `;
      ...
      const emailRes = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          from: { email: 'shop@replaybrick.com', name: 'Replay Bricks' },
          subject: `Order #${order.id} Confirmed — Replay Bricks`,
          content: [{ type: 'text/html', value: emailHtml }],
        }),
      });

      if (!emailRes.ok) {
        console.warn(`Order ${order.id}: Email send returned ${emailRes.status}`);
      }
    } catch (emailErr) {
      // Non-critical — order is still created
      console.warn(`Order ${order.id}: Email failed:`, emailErr.message);
    }
...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    const orders = await blobRawGet('clutch_orders');
    const items = await blobRawGet('clutch_order_items');
...
app.put('/api/orders/:id/status', async (req, res) => {
  ...
  const doHandle = async () => {
    ...
    const validStatuses = ['pending', 'paid', 'picked', 'packed', 'shipped', 'delivered', 'cancelled'];
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ error: `Invalid status: ${status}. Valid: ${validStatuses.join(', ')}` });
    }

    // Load order + its items
    const orders = await blobRawGet('clutch_orders');
    const order = orders.find(o => o.id === id);
    ...
    if (prevStatus === status) {
      return res.json({ ok: true, note: 'Status unchanged' });
    }

    // Load order items ONCE — filter by order_id after load
    let orderItems = [];
    try {
      const allOrderItems = await blobRawGet('clutch_order_items');
      orderItems = allOrderItems.filter(oi => oi.order_id === id);
    } catch {
      // Fallback: items might be inline on the order
      orderItems = order.items || [];
    }

    // ═══════════════════════════════════════════════════
    // OPTIMIZED: INVENTORY DEDUCTION on "picked"
    // ═══════════════════════════════════════════════════
    if (status === 'picked' && prevStatus !== 'picked') {
      if (orderItems.length === 0) {
        return res.status(400).json({ error: 'Order has no items — cannot pick without items' });
      }

      // Load inventory ONCE at the top of the deduction block
      const inventory = await blobRawGet('clutch_inventory');
      if (!Array.isArray(inventory) || inventory.length === 0) {
        return res.status(500).json({ error: 'Inventory data unavailable or empty' });
      }

      // Build inventory lookup by id for O(1) access
      const invById = new Map(inventory.map(i => [i.id, i]));
      // For part_no+color matching, build a composite key map
      const invByPartColor = new Map();
      for (const i of inventory) {
        if (i.part_no) {
          const key = ((i.part_no || '') + '|' + (i.color_id || '')).toLowerCase();
          if (!invByPartColor.has(key)) {
            invByPartColor.set(key, i);
          }
        }
      }
      ...
      for (const oi of orderItems) {
        // Find matching inventory: prefer inventory_id, fall back to part_no match
        let invItem = null;

        if (oi.inventory_id) {
          invItem = invById.get(oi.inventory_id) || null;
        }

        if (!invItem && oi.item_no) {
          // Try exact part_no + color_id match first
          const exactKey = ((oi.item_no || '') + '|' + (oi.color_id || '')).toLowerCase();
          invItem = invByPartColor.get(exactKey) || null;
          // Fall back to part_no-only match
          if (!invItem) {
            invItem = inventory.find(i => {
              const partMatch = (i.part_no || '').toLowerCase() === (oi.item_no || '').toLowerCase();
              if (!partMatch) return false;
              if (oi.color_id && i.color_id !== oi.color_id) return false;
              return true;
            });
          }
        }
        ...
        if (!invItem) {
          shortages.push({ item: oi.item_no || oi.item_name || 'unknown', reason: 'Not found in inventory' });
          continue;
        }

        const neededQty = oi.quantity || 1;
        const currentQty = invItem.quantity || 0;

        if (currentQty < neededQty) {
          shortages.push({
            item: `${invItem.part_no}${invItem.color_name ? ' (' + invItem.color_name + ')' : ''}`,
            reason: `Need ${neededQty}, have ${currentQty}`,
          });
          continue;
        }
      ...
      if (shortages.length > 0) {
        const msg = shortages.map(s => `${s.item}: ${s.reason}`).join('; ');
        return res.status(409).json({
          error: 'Insufficient inventory',
          shortages: shortages.map(s => s.item),
          details: msg,
        });
      }

      // ── Apply deductions using BATCH updates ──
      // For large orders (500+ items), process in progressive batches
      // so partial progress can be reported if time runs short.
      const BATCH_SIZE = 250;
      const totalDeductions = deductions.length;
      const isLargeOrder = totalDeductions >= 500;
      let completedCount = 0;

      for (let i = 0; i < totalDeductions; i += BATCH_SIZE) {
        const batch = deductions.slice(i, i + BATCH_SIZE);

        // Build inventory batch updates
        const inventoryUpdates = batch.map(d => ({
          id: d.invItem.id,
          data: {
            quantity: d.newQty,
            last_synced_at: new Date().toISOString(),
        ...
        // Link order items to inventory (batch)
        const orderItemUpdates = batch
          .filter(d => !d.orderItem.inventory_id)
          .map(d => ({
            id: d.orderItem.id,
            data: { inventory_id: d.invItem.id },
          }));
        if (orderItemUpdates.length > 0) {
          await blobBatchUpdate('clutch_order_items', orderItemUpdates);
        }

        completedCount += batch.length;

        // For large orders, check time remaining after each batch
        if (isLargeOrder && (i + BATCH_SIZE) < totalDeductions) {
          const elapsed = Date.now() - handlerStartMs;
          if (elapsed > 60000) {
            // 60+ seconds elapsed — commit status now and report partial progress
            const partialUpdates = {
              status,
              last_synced_at: new Date().toISOString(),
              updated_at: new Date().toISOString(),
            };
            if (status === 'paid' && !order.paid_date) partialUpdates.paid_date = new Date().toISOString().slice(0, 10);
            await blobUpdate('clutch_orders', id, partialUpdates);

            return res.json({
              ok: true,
              prevStatus,
              status,
              deducted: completedCount,
              pending: totalDeductions - completedCount,
              note: `Large order partially processed. ${completedCount} of ${totalDeductions} items deducted. Re-submit this request to continue processing the remaining items.`,
            });
          }
        }
      }
    ...
    // ═══════════════════════════════════════════════════
    // OPTIMIZED: INVENTORY RESTORE on "cancelled"
    // ═══════════════════════════════════════════════════
    if (status === 'cancelled' && (prevStatus === 'picked' || prevStatus === 'paid')) {
      if (orderItems.length > 0) {
        // Load inventory ONCE
        const inventory = await blobRawGet('clutch_inventory');
        const invById = new Map(inventory.map(i => [i.id, i]));
        ...
        for (const oi of orderItems) {
          if (oi.inventory_id) {
            const invItem = invById.get(oi.inventory_id);
            if (invItem) {
              const restoredQty = (invItem.quantity || 0) + (oi.quantity || 1);
        ...
        // Apply all restores in a single batch operation
        if (inventoryRestores.length > 0) {
          await blobBatchUpdate('clutch_inventory', inventoryRestores);
        }
      }
    }
    ...
    // Set date fields for status milestones
    if (status === 'paid' && !order.paid_date) updates.paid_date = new Date().toISOString().slice(0, 10);
    if (status === 'shipped' && !order.shipped_date) updates.shipped_date = new Date().toISOString().slice(0, 10);

    await blobUpdate('clutch_orders', id, updates);

    const result = { ok: true, prevStatus, status };
    if (status === 'picked') {
      result.deducted = totalDeductions || deductions?.length || 0;
    }
    if (status === 'cancelled') {
  ...
  // Wrap the entire handler in a timeout guard (80s → leaves 10s for Netlify overhead)
  try {
    await withTimeout(doHandle(), 80000, 'PUT /api/orders/:id/status');
  } catch (err) {
    if (err.message && err.message.includes('timed out')) {
      console.error('[hold] Status update TIMEOUT:', err.message);
      return res.status(504).json({
        error: 'The operation could not complete within the time limit. For large orders, try again — partial progress may have been saved.',
      });
    }
    console.error('[hold] Status update error:', err);
...
app.get('/api/dashboard', async (req, res) => {
  try {
    const inventory = await blobRawGet('clutch_inventory');
    const orders = await blobRawGet('clutch_orders');
    const syncLogs = await blobRawGet('clutch_sync_log');
...
app.get('/api/pricing-rules', async (req, res) => {
  try {
    const rules = await blobRawGet('clutch_pricing_rules');
...
app.post('/api/pricing-rules', async (req, res) => {
  try {
    ...
    const newRule = await blobInsert('clutch_pricing_rules', {
      name,
      rule_type,
...
app.delete('/api/pricing-rules/:id', async (req, res) => {
  try {
    await blobDelete('clutch_pricing_rules', parseInt(req.params.id));
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/pending', async (req, res) => {
  try {
    const pendingItems = await blobRawGet('clutch_pending_items');
    const colors = await blobRawGet('clutch_bl_colors');
...
app.post('/api/pending', async (req, res) => {
  try {
    ...
    const newItem = await blobInsert('clutch_pending_items', {
      part_no,
...
app.put('/api/pending/:id', async (req, res) => {
  try {
    ...
    const success = await blobUpdate('clutch_pending_items', id, updates);
    if (!success) return res.status(404).json({ error: 'Not found' });
...
app.delete('/api/pending/:id', async (req, res) => {
  try {
    await blobDelete('clutch_pending_items', parseInt(req.params.id));
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/pending/confirm', async (req, res) => {
  try {
    ...
    await blobRawSet('clutch_inventory', inventory);
    await blobRawSet('clutch_pending_items', pendingItems);
...
app.get('/api/settings', async (req, res) => {
  try {
    const settings = await blobRawGet('clutch_settings');
    const out = {};
...
app.put('/api/settings', async (req, res) => {
  try {
    ...
    await blobRawSet('clutch_settings', settings);
...
app.get('/api/pricing/preview', async (req, res) => {
  try {
    ...
    const inventory = await blobRawGet('clutch_inventory');
    const priceCache = await blobRawGet('clutch_price_cache');
...
app.get('/api/picking', async (req, res) => {
  try {
    ...
    const orderItems = await blobRawGet('clutch_order_items');
    const orders = await blobRawGet('clutch_orders');
    const inventory = await blobRawGet('clutch_inventory');
...
app.post('/api/partout/:setNo/confirm', async (req, res) => {
  try {
    ...
    const pending = await blobRawGet('clutch_pending_items');
    ...
    await blobRawSet('clutch_pending_items', pending);
...
const PUSH_LOG_KEY = 'clutch_push_log';
...
app.post('/api/reconcile/:orderId', async (req, res) => {
  try {
    ...
    if (result) {
      ...
      await blobRawSet('clutch_orders', orders);
    }
...
async function reconcileOrderInternal(order, mode) {
  ...
  if (mode === 'live' && changes.length > 0) {
    await blobRawSet('clutch_marketplace_lots', lots);
  }
...
app.post('/api/scheduler/tick', async (req, res) => {
  ...
  try {
    ...
    try {
      const stateBlob = await blobRawGet('clutch_cron_state').catch(() => ({}));
      ...
      await blobSet('clutch_cron_state', { lastOrderIds: orders.map(o => o.id), lastRun: new Date().toISOString() });
    } catch (e) {
...
app.post('/api/scheduler/backup', async (req, res) => {
  try {
    const keys = ['clutch_inventory', 'clutch_orders', 'clutch_order_items', 'clutch_marketplace_lots', 'clutch_bl_colors', 'clutch_api_credentials', 'clutch_pricing_rules', 'clutch_sync_log', 'clutch_price_cache', 'clutch_pending_items', 'clutch_settings', 'clutch_push_log'];
